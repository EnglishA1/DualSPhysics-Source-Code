...
public:
  ...
  ///Structure to manage the information of the excluded particles.
  typedef struct{ 
    unsigned part;
    float timeout;
    unsigned id;
...
public:
  ...
  unsigned SetDataUnsorted(unsigned part,float timepart,bool outreset,unsigned npok,unsigned nout,const unsigned* id,const tfloat3* pos,const tfloat3* vel,const float* rhop,const float* press, const float* viscop, const tsymatrix3f* Vtau ,const unsigned* idpm);

  unsigned GetDataSort(unsigned size,unsigned *id,tfloat3 *pos,tfloat3 *vel,float *rhop,bool full)const;
  void GetDataPointers(unsigned* &id,tfloat3* &pos,tfloat3* &vel,float* &rhop,float* &press,float* &viscop,tsymatrix3f* &vtau,unsigned* &idpm ){ id=Id; pos=Pos; vel=Vel; rhop=Rhop; press=Press; viscop=Viscop; vtau=Vtau; idpm=Idpm;}
  ...
  void SaveFileAscii(std::string file="",bool extrafiles=true)const;
