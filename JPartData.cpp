...
JPartData::JPartData(){
  ...
  Out=NULL;
  //multi
  Press=NULL; Viscop=NULL; Idpm=NULL; Vtau=NULL;
...
void JPartData::Config(TpFmtFile fmt,unsigned np,unsigned nbound,unsigned nfluid,unsigned nfixed,unsigned nmoving,unsigned nfloat,float dp,float h,float b,float rhop0,float gamma,float massbound,float massfluid,bool data2d){
  ...
  try{
	...
	Press=new float[Np];       
	Viscop=new float[Np];       
	Idpm=new unsigned[Np];       
	Vtau=new tsymatrix3f[Np];
  }
...
unsigned JPartData::GetMemoryAlloc()const{
  ...
  //Addtitional
  if(Press)m+=Np*sizeof(float);
  if(Viscop)m+=Np*sizeof(float);
  if(Idpm)m+=Np*sizeof(unsigned);
  if(Vtau)m+=Np*sizeof(tsymatrix3f);
...
/// Returns true if no particles were excluded, when previous data were.
//==============================================================================
unsigned JPartData::SetDataUnsorted(unsigned part,float timepart,bool outreset,unsigned npok,unsigned nout,const unsigned* id,const tfloat3* pos,const tfloat3* vel,const float* rhop,const float* press, const float* viscop, const tsymatrix3f* vtau ,const unsigned* idpm){
  const char met[]="SetDataUnsorted";
  unsigned errout=0;
  if(!DataConfigOk())RunException(met,"Initial configuration is missing.");
  if(NfluidOut!=OutCount)RunException(met,"THe number of excluded particles does not match with the stored number.");
  if(npok+nout>Np)RunException(met,"The number of partiples is larger than the maximum.");
  if(!id||!pos||!vel||!rhop)RunException(met,"Particle data are missing.");
  SetPartNumber(part);
  SetPartTime(timepart);
  if(outreset)FluidOutClear(true);
  if(GetFmtData()==FmtBi2){
    const unsigned npokold=Np-NfluidOut;
    //-Generates indexes of particles in id [] from particle identifier.
    int *rid=new int[Np];
    ...
    for(unsigned p=0;p<Nbound;p++){
      int pid=rid[p]; 
      if(pid>=0){ Pos[p]=pos[pid]; Vel[p]=vel[pid]; Rhop[p]=rhop[pid]; Press[p]=press[pid]; Viscop[p]=viscop[pid]; Vtau[p]=vtau[pid]; Idpm[p]=idpm[pid]; } //multi
    }
#ifdef DBG_JPartData
    ...
    for(unsigned p=Nbound;p<Np;p++){
      ...
      if(OrderOut[p-Nbound]==-1){//-No excluded particles.
        if(pid>=0){
          Id[pok]=p; Pos[pok]=pos[pid]; Vel[pok]=vel[pid]; Rhop[pok]=rhop[pid]; Press[pok]=press[pid]; Viscop[pok]=viscop[pid]; Vtau[pok]=vtau[pid]; Idpm[pok]=idpm[pid]; pok++; //multi
        }
        else{
          ...
          if(pokold!=pok){ Id[pok]=p; Pos[pok]=Pos[pokold]; Vel[pok]=Vel[pokold]; Rhop[pok]=Rhop[pokold]; Press[pok]=Press[pokold]; Viscop[pok]=Viscop[pokold]; Vtau[pok]=Vtau[pokold]; Idpm[pok]=Idpm[pokold]; } //multi
          pok++;
        } 
      } 
      else if(pid>=0&&pid<int(npok))errout++;//-Excluded particle that now they are not...
    }
    delete[] rid;
  ...
  else RunException(met,"This method is not implementd for the format BINX-20.");
  DataBound=true;
  return(errout);
}
...
//==============================================================================
void JPartData::SaveFileAscii(std::string file,bool extrafiles)const{
  const char met[]="SaveFileAscii";
  if(!DataConfigOk())RunException(met,"Initial configuration is missing.");
  if(!DataBoundOk())RunException(met,"Boundary particles data are missing.");
  if(NfluidOut>OutCount)RunException(met,"Excluded particles data are missing.");
...
void JPartData::LoadFileBin(unsigned part,const std::string &file){
  ...
  if(pf){
    ...
    if(inf.fmt!=11)RunException(met,"Invalid file format 'LoadFileBin'.",file);
    ...
    if(!DataBoundOk()&&!inf.full)RunException(met,"Fixed boundary data are missing.",file);
    ...
    pf.close();
    //-Checks if any particle cease to be excluded and it is excluded again.
    if(1){
      for(unsigned c=Nbound;c<inf.npok;c++)if(OrderOut[Id[c]-Nbound]>=0){
        printf("The particle %d was OUT and now it is not...\n",Id[c]);
        inf.npok--;
        //-Particle sent to the end of the arrays to be converted again in Out.
...
void JPartData::SortDataOut(){
  ...
  if(!DataConfigOk())RunException(met,"Initial confoguration is missing.");
  if(!DataBoundOk())RunException(met,"Boundary particle data are missing.");
  if(NfluidOut>OutCount)RunException(met,"Excluded particle data are missing.");
  for(unsigned c=0;c<NfluidOut;c++){//-Places data of excluded particles in the main arrays.
...
JPartData::StInfoFileBi2 JPartData::GetInfoFileBi2(ifstream &pf,bool fileout)const{
  ...
  StHeadFmtBin hfmt;
  pf.read((char*)&hfmt,sizeof(StHeadFmtBin));
  //printf("hfmt.titu=%s",hfmt.titu);
  //printf("\nhfmt.titu=%d",!strcmp(hfmt.titu,"#FileJBD JPartD "));
  //getchar();
...
void JPartData::LoadFileBi2(unsigned part,const std::string &file){
  ...
  if(pf){
    ...
    if(inf.fmt!=20)RunException(met,"Invalid file format 'LoadFileBi2'.",file);
    ...
    if(inf.nfluidout){
      //-Labels the excluded particles in orderout.
...
void JPartData::SaveFileBi2(bool full,std::string file)const{
  ...
  if(!DataConfigOk())RunException(met,"Initial configuration is missing.");
  if(!DataBoundOk())RunException(met,"Boundary pasticles data are missing.");
...
void JPartData::LoadFileBi2Out(std::string file){
  ...
  if(pf){
    ...
    if(inf.fmt!=20)RunException(met,"Invalid file format. 'LoadFileBi2Out'",file);
    unsigned nbound=inf.nfixed+inf.nmoving+inf.nfloat;
    unsigned nfluid=inf.np-nbound;
    if(!DataConfigOk())Config(FmtBi2,inf.np,nbound,nfluid,inf.nfixed,inf.nmoving,inf.nfloat,inf.dp,inf.h,inf.b,inf.rhop0,inf.gamma,inf.massbound,inf.massfluid,inf.data2d!=0);
    else{  
...
void JPartData::SaveFileBi2Out(std::string file,bool updatebi2out){
  ...
  if(!DataConfigOk())RunException(met,"iInitial configuration is missing.");
  if(!DataBoundOk())RunException(met,"Boundary particles data are missing.");
  if(NfluidOut>OutCount)RunException(met,"Excluded particles data are missing.");
...
JPartData::StInfoFileBin JPartData::GetInfoFromFileBin(const std::string &file)const{
  ...
  if(pf){
    ...
    if(inf.fmt!=11)RunException(met,"Invalid file format. 'GetInfoFromFileBin'",file);
    pf.close();
  }
  else RunException(met,"Cannot open the file.",file);
  return(inf);
...
JPartData::StInfoFileBi2 JPartData::GetInfoFromFileBi2(const std::string &file)const{
  ...
  if(pf){
    ...
    if(inf.fmt!=20)RunException(met,"Invalid file format. 'GetInfoFromFileBi2'",file);
    pf.close();
  }
  else RunException(met,"Cannot open the file.",file);
  return(inf);
...
unsigned JPartData::GetDataSort(unsigned size,unsigned *id,tfloat3 *pos,tfloat3 *vel,float *rhop,bool full)const{
  ...
  if(!DataBoundOk())RunException(met,"Boundary particles data are missing.");
  if(full&&NfluidOut>OutCount)RunException(met,"Excluded pasticle data are missing.");
  if(FmtData==FmtAscii){
