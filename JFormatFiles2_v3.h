...
public:
  ...
  //============================================================================== 
  static void SaveVtkBasic(std::string fname,unsigned np,const unsigned* idp,const tfloat3* pos,const tfloat3* vel,const float* rhop,const float* press,const float *viscop, tsymatrix3f* vtau,const unsigned *idpm){
    StScalarData fields[9];
    unsigned nfields=0;
    if(idp){  fields[nfields]=DefineField("Id",UInt32,1,idp);     nfields++; }
	if(pos){  fields[nfields]=DefineField("Pos",Float32,3,pos);   nfields++; }
	...
	//multi
	if(press){	fields[nfields]=DefineField("Press",Float32,1,	press); nfields++; }
	if(viscop){	fields[nfields]=DefineField("Visco",Float32,1,	viscop);nfields++; }
	if(idpm){	fields[nfields]=DefineField("Id_m",UInt32,1,	idpm);  nfields++; }
	//if(vtau){	fields[nfields]=DefineField("Viscous tau",Float32,3,	vtau);  nfields++; }
