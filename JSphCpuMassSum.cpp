...
float JSphCpu::GetKernelWab(float rr2)const{
  const float rad=sqrt(rr2);
	const float qq=rad/H;
	const float wqq1=1.f-0.5f*qq;
	const float wqq2=2*qq+1.f;
	float kernelValue=Awen*wqq1*wqq1*wqq1*wqq1*wqq2; //1.f-0.5f*qq;
	return kernelValue;
}
          ...
          if(rr2<=Fourh2 && rr2>=ALMOSTZERO){
            ...
            if(compute){
              ...
              //if(compute)arp1+=massp2*(dvx*frx+dvy*fry+dvz*frz);
							arp1+=massp2*GetKernelWab(rr2); //ADDED
  ...
  ,TpShifting tshifting,tfloat3 *shiftpos,float *shiftdetect)const
{
  ...
  if(npbok){
    ...
    InteractionForcesBound      <psimple,tker,ftmode> (npbok,0,nc,hdiv,cellfluid,begincell,cellzero,dcell,pos,pspos,velrhop,code,idp,viscdt,ar);
		//-Interaction of type Bound-Fluid / Interaccion Bound-Bound
    InteractionForcesBound      <psimple,tker,ftmode> (npbok,0,nc,hdiv,0				,begincell,cellzero,dcell,pos,pspos,velrhop,code,idp,viscdt,ar);
  }
}
...
template<bool shift> void JSphCpu::ComputeSymplecticPreT(double dt){
  ...
  for(int p=0;p<npb;p++){
    ...
    //const float rhopnew=float(double(vr.w)+dt05*Arc[p]);
		const float rhopnew=Arc[p]; //ADDED
